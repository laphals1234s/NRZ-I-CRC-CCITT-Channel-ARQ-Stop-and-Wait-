...
with st.expander("Chi tiết khung & CRC (TX vs RX + highlight lỗi)"):
    ...
    if err_idx0:
        st.write(f"**Vị trí bit sai (0-based, bắt đầu tại bit 0):** {err_idx0}")
    else:
        st.write("**Vị trí bit sai (0-based, bắt đầu tại bit 0):** —")
    ...
    # ----- Chuỗi ký tự thu được (RX) -----
    # Giải chuỗi ký tự theo bên thu từ M_len bit đầu của RX (bỏ FCS):
    try:
        st.markdown("**Chuỗi ký tự thu được (RX, giải từ M bit đầu):**")
        st.code(rx_text, language='text')
    except Exception as _e:
        st.write("Không thể giải chuỗi ký tự RX.")
