...
# ============================================
# Channel: ABNC (Additive Bounded-Noise Channel)
#   y = x + n, with |n| ≤ N.
#   SNR (dB) = 20*log10(Vs/N), Vs = V_high - V_low.
#   RX generated by clipping a zero-mean Gaussian to ±N (random-but-bounded).
# ============================================
def abnc(signal, snr_db, V_low, V_high):
    x = signal.astype(float)
    Vs = float(V_high - V_low)
    N = Vs / (10.0 ** (snr_db / 20.0)) if Vs != 0 else 0.0
    g = np.random.normal(0.0, N if N > 0 else 1e-12, size=x.shape)
    noise = np.clip(g, -N, N)
...
with st.sidebar:
    ...
    st.header("Channel")
    ch_type = st.selectbox("Loại kênh", ["ABNC (|n| ≤ N)"])
    ...
    st.caption("**Additive Bounded-Noise Channel (ABNC)** – **Kênh nhiễu bị chặn biên (|n| ≤ N)**.  Công thức: SNR = 20·log10(Vs/N), N = Vs/10^(SNR/20).")
...
plot_stairs(axes[1], y_rx, "RX waveform (ABNC)", "Voltage (V)")
...
Text (6 chars)
  ...
  Dạng sóng mức (TX)
       ↓  Kênh (ABNC: |n| ≤ N), SNR = 20·log10(Vs/N)
  Dạng sóng mức (RX)
